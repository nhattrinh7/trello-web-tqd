...
function AutoCompleteSearchBoard() {
  ...
  const handleInputSearchChange = (event) => {
    ...
    // Gọi API...
    setLoading(true) // khi gõ text để search thì hiện loading
    fetchBoardsAPI(searchPath)
      .then(res => {
        setBoards(res.boards || []) // setBoard để lấy dữ liệu hiển thị ra, đương nhiên là phía dưới chỗ getOptionLabel giúp chỉ hiện thị ra title của board thôi
      })
      .finally(() => {
        setLoading(false) // thành công hay thất bại thì cũng set nó về false
      })
  }
  // Bọc hàm handleInputSearchChange vào useDebounceFn và cho delay 0.5 giây khi dừng gõ phím thì mới chạy function, rôi nhét vào props onInputChange
    ...
    <Autocomplete
      ...
      renderInput={(params) => (
        <TextField
          ...
          slotProps={{
            input: {
              ...params.InputProps, // cái này là cái gì nhỉ
              startAdornment: (
